// FallLink · reusable WebRTC P2P library for the AI-Native Solutions estate
// MIT · sjgant80@gmail.com · https://github.com/sjgant80-hub/falllink
//
// One connection layer every estate tool imports.
// STUN + BroadcastChannel (same-origin auto) + manual offer/answer paste (cross-network).
...
const DEFAULT_BOOTSTRAP = []; // estate peers list (populate per tool)
...
export class FallLink extends EventTarget {
  constructor(opts = {}) {
    super();
    ...
    this.peers = new Map(); // peerId -> peerWrapper
    this._bc = null;
    this._broadcasting = false;
    this._pingIntervalMs = opts.pingIntervalMs || 3000;
  }

  // ---- events (convenience wrapper over EventTarget) ----
  on(type, fn) {
  ...
  _handleMessage(wrapper, data) {
    ...
    if (parsed && parsed.__fl === 'pong') {
      wrapper.latency = Math.round(performance.now() - parsed.t);
